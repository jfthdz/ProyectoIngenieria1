...
function validarFormularioModificarOferta() {
    try {
        event.preventDefault();
        const opcionPrivada = document.querySelector('label[for="privado"]');
        const opcionPublica = document.querySelector('label[for="publico"]');
...
async function updatePuesto(){
    const puestoJSON = sessionStorage.getItem("ofertaSeleccionada");
    const puestoObject = JSON.parse(puestoJSON);
    const puestoId = puestoObject._id;

    const puestoData = new FormData(document.querySelector("#form-modificar-puesto"));
    puestoData.append("puestoId", puestoId);
...
async function eliminarOfertaSeleccionada(puestoId){
    event.preventDefault();
    const puestoData = new FormData();
    puestoData.append("puestoId", puestoId);

    const url = "/puestos/deletePuesto"
